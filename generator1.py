...
class DataGenerator(Sequence):
	...
	def _get_box_anchor(self, obj):
		...
		return box, best_anchor, grid_y, grid_x, obj_indx

	def show_img(self, img, all_obj):

		for obj in all_obj:
			cv2.rectangle(img, (obj['xmin'],obj['ymin']), (obj['xmax'],obj['ymax']), (155,120*int(obj['name']),0), 3)
			cv2.putText(img, obj['name'], (obj['xmin']+2, obj['ymin']+12),  0, 1.2e-3 * img.shape[0],  (155,120*int(obj['name']),0), 2)
			plt.imshow(img)
	...
	def __data_generation(self, l_bound, r_bound):
		...
		x_batch = np.zeros((r_bound - l_bound, self.config['IMAGE_H'], self.config['IMAGE_W'], 1))                         # input images
		b_batch = np.zeros((r_bound - l_bound, 1     , 1     , 1    ,  self.config['TRUE_BOX_BUFFER'], 4))   # list of self.config['TRUE_self.config['BOX']_BUFFER'] GT boxes
		y_batch = np.zeros((r_bound - l_bound, self.config['GRID_H'],  self.config['GRID_W'], self.config['BOX'], 4+1+len(self.config['LABELS'])))                # desired network output


		# print len(self.imgs[l_bound:r_bound])
		# print "imgs"+str(len(self.imgs[l_bound:r_bound]))
		...
		for train_instance in self.imgs[l_bound:r_bound]:
			...
			x_batch[instance_count] = img[:,:,1].reshape((img.shape[0],img.shape[1],1)) 

			# print np.sum(np.count_nonzero(img))
			...
			for obj in all_obj:
				if obj['xmax'] > obj['xmin'] and obj['ymax'] > obj['ymin'] and obj['name'] in self.config['LABELS']:
					...
					if grid_x < self.config['GRID_W'] and grid_y < self.config['GRID_H']:
						...
						# assign ground truth x, y, h, w, confidence and class probs to y_batch
						...
						# print best_anchor
						# print y_batch[instance_count, grid_y, grid_x, best_anchor, :]
	...
	def __getitem__(self, idx):
		...
		# print "Barch"+str(idx)+ ": l:" + str(l_bound) + "r:" + str(r_bound)
